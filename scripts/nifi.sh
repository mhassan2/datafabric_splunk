...
printf "\033[1;34mThis [$scriptNAME] is the official method to start/stop/run Nifi. While you can still use this script, it is recommend that you use \033[0;32m/etc/init.d/nifi [start|stop|restart]\033[0m \033[1;34m warpper script instead. All startup scritps in /etc/init.d has been modified to work with this container. "
printf "\033[1;34mIf you continue to use; some non-critical operations may not work properly, like:\n-syncing with superisord (http://localhost:9001)\n-Preventing startup if ENV var set NIFI=NO (by docker run)\033[m\n"
echo
